...
export default function Ielts() {
    ...
    const course = useMemo(() => (
        ...
        courseCatalog.find((item) => item.id === 'ielts') ||
        null
    ), [selectedCourseKey]);
    ...
    const handlePracticeMaterialEnroll = (material) => {
        ...
        sessionStorage.setItem('ieltsEnrollmentSelection', JSON.stringify(selection));
        navigate(`/enroll?course=${selection.courseId}&country=${geoCountry}&type=PM`, {
            state: { courseEnrollmentSelection: selection, ieltsEnrollmentSelection: selection },
        });
    };
    ...
    const buildEnrollmentSelection = (tabKey) => {
        ...
        return {
            courseId: course?.id || 'ielts', programName: 'IELTS', tab: tabKey, country: geoCountry,
    ...
    return (
        <main>
            <section className="course-detail">
                <div className="course-detail-tabs" role="tablist" aria-label="IELTS programs">
                    <button type="button" className={`course-detail-tab ${activeTab === 'online' ? 'is-active' : ''}`} role="tab" id="ielts-online-tab" aria-selected={activeTab === 'online'} aria-controls="ielts-online-panel" onClick={() => setActiveTab('online')}>Online</button>
                    <button type="button" className={`course-detail-tab ${activeTab === 'selfLearning' ? 'is-active' : ''}`} role="tab" id="ielts-self-learning-tab" aria-selected={activeTab === 'selfLearning'} aria-controls="ielts-self-learning-panel" onClick={() => setActiveTab('selfLearning')}>Self Learning</button>
                    <button type="button" className={`course-detail-tab ${activeTab === 'materials' ? 'is-active' : ''}`} role="tab" id="ielts-materials-tab" aria-selected={activeTab === 'materials'} aria-controls="ielts-materials-panel" onClick={() => setActiveTab('materials')}>Materials</button>
                ...
                <div className="course-detail-grid">
                    ...
                    <div className="course-detail-content">
                        <div role="tabpanel" id="ielts-online-panel" aria-labelledby="ielts-online-tab" hidden={activeTab !== 'online'}>{renderTabContent('online', 'OL')}</div>
                        <div role="tabpanel" id="ielts-self-learning-panel" aria-labelledby="ielts-self-learning-tab" hidden={activeTab !== 'selfLearning'}>{renderTabContent('selfLearning', 'SL')}</div>
                        <div role="tabpanel" id="ielts-materials-panel" aria-labelledby="ielts-materials-tab" hidden={activeTab !== 'materials'}>{renderPracticeMaterial()}</div>
                    </div>
                </div>
            </section>
        </main>
